    ...
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        ...
        const timeoutId = setTimeout(() => {
          ...
          killProcess(pasteProcess, "SIGKILL");
          pasteProcess.removeAllListeners();
          // windows-fast-paste.c prints PASTE_OK immediately after SendInput
          // succeeds, then exits -- a timeout here usually means the process
          // was just slow to tear down, not that the keystroke never landed.
          // Falling back and re-sending Ctrl+V in that case pastes twice.
          if (stdoutData.includes("PASTE_OK")) {
            this.safeLog("⏱️ Windows fast-paste slow to exit after PASTE_OK, treating as success");
            resolve({
              restoreComplete:
